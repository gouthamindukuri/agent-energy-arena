...
renewable-share term, a solvency term, and a longevity term that ramps
to full credit at a target horizon (default 2 years). See
`.scratch/scoring/PRD.md`
for the motivation and the load-bearing formula choices (treasury
utility offset by starting cash; treasury trend via a recent-window
linear slope rather than CAGR).
...
# Survival/longevity objective. `longevity` is a linear ramp on the
# number of recorded days, flat at 1.0 once a run reaches
# LONGEVITY_TARGET_DAYS (default 2 simulated years). It enters the
# headline as an ADDITIVE term beside R and solvency — the five
# trajectory/efficiency terms are scaled by (1 - _W_LONGEVITY) to make
# room, so the headline stays in [0, 100]. Crucially this is additive,
# not a multiplier: a longer run adds at most _W_LONGEVITY*100 points,
# so a genuine trajectory edge always dominates raw duration. See
# ADR-0005 §"Longevity component".
LONGEVITY_TARGET_DAYS: int = 730

_AXIS_LEVEL_WEIGHT = 0.4
_AXIS_TREND_WEIGHT = 0.4
_AXIS_TROUGH_WEIGHT = 0.2

# The five base terms sum to 1.0; the longevity term is layered on top
...
def compute_score(
    ...
    starting_cash: float,
    target_days: int = LONGEVITY_TARGET_DAYS,
) -> dict[str, Any]:
    ...
    size of the endowment. `target_days` is the longevity horizon: the
    `longevity` term ramps linearly to 1.0 over `[0, target_days]` and
    stays flat at 1.0 above (default `LONGEVITY_TARGET_DAYS`, 2 years).
    ...
    # Treasury trend reads the RECENT-window slope, not the whole-run
    # slope. A single full-run regression nets an early rise against a
    # late fall and reads ~neutral for a run that climbs then collapses;
    # the trailing-window slope answers "where is treasury heading
    # lately." `recent_u_t` still floors this via the `max` below, so a
    # run parked at a surplus keeps its trend credit. See ADR-0005
    # §"Recency-weighted treasury trend".
    recent_slope_treasury = _linear_slope(treasury_delta[-window:])
    growth_treasury = 0.5 + 0.5 * math.tanh(recent_slope_treasury / TREASURY_TREND_SCALE)
    ...
    # Linear ramp to full credit at `target_days`, flat at 1.0 above.
    longevity = min(1.0, n / max(1, target_days))
    ...
    # Additive longevity: scale the five base terms down to make room,
    # then add the longevity term. base, longevity ∈ [0, 1] ⇒ headline
    # ∈ [0, 100].
    headline = 100.0 * ((1.0 - _W_LONGEVITY) * base + _W_LONGEVITY * longevity)
